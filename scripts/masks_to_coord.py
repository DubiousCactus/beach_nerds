...
def main(labels_path, masks_path, output_path, test):
    labels = {}
    ...
    for input_img, ground_truth in labels.items():
        labels[input_img] = []
        # Load the mask image
        for bbox, mask_file in zip(ground_truth['boxes'], ground_truth['masks']):
            mask_path = os.path.join(masks_path, "train" if not test else "test", input_img.split(".")[0], mask_file)
            # Convert to coordinates
            mask_img = np.asarray(Image.open(mask_path).convert("L"))
            r_rect, coords = convert_mask_to_coords(mask_img)
            # Save in dict
            labels[input_img].append((r_rect, coords, bbox))
...
if __name__ == "__main__":
    ...
    main(args.labels_path, args.masks_path, args.output_path, args.test)
