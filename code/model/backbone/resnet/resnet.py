...
class BasicBlock(xnn.Module):
    ...
    def forward(self, x):
        return nn.ReLU(inplace=True)(
            self.body(x) + (x if self.downsample is None else self.downsample(x))
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if radix > 1:
            ...
            body.append(
                xnn.Conv2d(
                    group_width, 3, stride, dilation, dilation, cardinality, bias=False
                )
            )
...
class Backbone(xnn.Module):
    def __init__(
        ...
        if deep_stem:
            head = [
                ...
                nn.ReLU(inplace=True),
                xnn.Conv2d(
                    stem_width * 2, kernel_size=3, stride=1, padding=1, bias=False
                ),
            ]
        else:
    ...
    def init(self):
        if self.name in weights and self.load_pretrained:
            print("load pre-training weights for", self.name)
            if self.name == "resnet50":
                import torchvision.models as models

                weight = models.ResNet50_Weights.IMAGENET1K_V2.get_state_dict(True)
            elif self.name == "resnet101":
                import torchvision.models as models

                weight = models.ResNet101_Weights.IMAGENET1K_V2.get_state_dict(True)
            else:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, is_first=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if self.avg_down:
                ...
                down_layers.append(
                    xnn.Conv2d(
                        planes * block.expansion, kernel_size=1, stride=1, bias=False
                    )
                )
            else:
                down_layers.append(
...
def resnet18(fetch_feature=False, pretrained=True):
    return Backbone(BasicBlock, (2, 2, 2, 2), "resnet18", fetch_feature, pretrained=pretrained)


def resnet34(fetch_feature=False, pretrained=True):
    return Backbone(BasicBlock, (3, 4, 6, 3), "resnet34", fetch_feature, pretrained=pretrained)


def resnet50(fetch_feature=False, pretrained=True):
    return Backbone(Bottleneck, (3, 4, 6, 3), "resnet50", fetch_feature, pretrained=pretrained)


def resnet101(fetch_feature=False, pretrained=True):
    return Backbone(Bottleneck, (3, 4, 23, 3), "resnet101", fetch_feature, pretrained=pretrained)


def resnet152(fetch_feature=False, pretrained=True):
    return Backbone(Bottleneck, (3, 8, 36, 3), "resnet152", fetch_feature, pretrained=pretrained)
