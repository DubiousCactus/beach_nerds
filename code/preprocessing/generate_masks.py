...
for image_fname in tqdm.tqdm(train_data.keys()):
    ...
    # Open image
    pil_image = Image.open(os.path.join(processed_dir, "train", image_fname)).convert(
        "RGB"
    )
    ...
    for idx, barcode in enumerate(barcodes):
        ...
        contours = [
            np.array([[point["x"], point["y"]] for point in barcode], dtype=np.int32)
        ]
        for cnt in contours:
        ...
        if not os.path.isdir(
            os.path.join(data_dir, "masks", "train", image_fname.split(".")[0])
        ):
            os.makedirs(
                os.path.join(data_dir, "masks", "train", image_fname.split(".")[0])
            )
        ...
        pil_mask.save(
            os.path.join(
                data_dir, "masks", "train", image_fname.split(".")[0], f"{idx}.jpg"
            )
        )
...
with open(
    os.path.join(data_dir, "json", "challenge", "train_challenge.json"), "w"
) as j:
    j.write(json_object)
...
for image_fname in tqdm.tqdm(test_data.keys()):
    ...
    for idx, barcode in enumerate(barcodes):
        ...
        # Save mask
        if not os.path.isdir(
            os.path.join(data_dir, "masks", "test", image_fname.split(".")[0])
        ):
            os.makedirs(
                os.path.join(data_dir, "masks", "test", image_fname.split(".")[0])
            )
        ...
        pil_mask.save(
            os.path.join(
                data_dir, "masks", "test", image_fname.split(".")[0], f"{idx}.jpg"
            )
        )
